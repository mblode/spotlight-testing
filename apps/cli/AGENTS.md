...
- `npm run test` — `vitest run` (25 tests across 7 files)
...
- **Minimal default UX**: Keep `on` and `off` terse. Rich session detail belongs in `spotlight-testing status`, not the default startup flow.

...
describe("git helpers", () => {
  test("compares linked worktrees by shared common dir", () => {
    ...
    try {
      expect(readGitCommonDir(fixture.root)).toBe(
        readGitCommonDir(fixture.worktree)
      );
  ...
  test("finds the primary checkout for a linked worktree", () => {
    ...
    try {
      expect(getMainWorktreeRoot(fixture.worktree)).toBe(
  ...
  test("detects busy repo states from git markers", () => {
    ...
    try {
      ...
      writeFileSync(
        mergeHead,
        `${execGit(fixture.root, ["rev-parse", "HEAD"])}\n`,
        "utf8"
      );

  ...
  () => {
    ...
    test("spotlight-testing on/off syncs and restores the target", async () => {
      ...
      try {
        ...
        expect(
          execGit(fixture.root, ["rev-parse", "--abbrev-ref", "HEAD"])
        ).toBe("main");
    ...
    test("spotlight-testing reset --to resets to the requested ref and cleans target files", async () => {
      const fixture = createRepoFixture({
        "app.txt": "initial\n",
      });
      const lockfilePath = join(fixture.parent, "spotlight.lock");
      ...
      try {
        ...
        const stopResult = spawnSync(
          "node",
          [cliPath, "reset", "--to", "main", "--no-fetch"],
          {
    ...
    test("spotlight-testing survives repeated SIGINT during restore cleanup", async () => {
      ...
      try {
        ...
        await waitFor(
          () =>
            readTextFileIfExists(fixture.root, getBatchFilePath(0)) ===
              "updated-0" &&
            readTextFileIfExists(
              fixture.root,
              getBatchFilePath(fileCount - 1)
            ) === `updated-${fileCount - 1}`,
          60_000
        );
        ...
        await waitFor(() =>
          getSpotlightOutput().includes("Stopping spotlight...")
        );
        await delay(5);
        killProcessGroup(spotlightProcess, "SIGINT");
    ...
    test("spotlight-testing keeps unrelated target runtime files during active resyncs", async () => {
      ...
      try {
        await waitFor(
          () =>
            existsSync(lockfilePath) &&
            readTextFile(fixture.root, "app.txt") === "from-worktree-one"
        );
        ...
        await waitFor(
          () => readTextFile(fixture.root, "app.txt") === "from-worktree-two"
        );

        expect(readTextFile(fixture.root, "runtime-artifact.txt")).toBe(
          "keep-me"
        );
        ...
        await waitFor(() => !existsSync(lockfilePath));
        expect(
          readTextFileIfExists(fixture.root, "runtime-artifact.txt")
        ).toBeNull();
      } finally {
        if (spotlightProcess.exitCode === null) {
          spotlightProcess.kill("SIGKILL");
    ...
    test("spotlight-testing removes worktree-owned files when they are deleted mid-session", async () => {
      ...
      try {
        await waitFor(
          () =>
            existsSync(lockfilePath) &&
            readTextFile(fixture.root, "app.txt") === "from-worktree"
        );

        writeTextFile(fixture.worktree, "notes.txt", "ephemeral\n");
        await waitFor(
          () => readTextFileIfExists(fixture.root, "notes.txt") === "ephemeral"
        );

        rmSync(join(fixture.worktree, "notes.txt"), { force: true });
        await waitFor(
          () => readTextFileIfExists(fixture.root, "notes.txt") === null
        );
    ...
    test("spotlight-testing infers the main checkout from an explicit worktree path", async () => {
      ...
      const spotlightProcess = spawn(
        "node",
        [cliPath, "on", fixture.worktree, "--debounce", "50"],
        {
          cwd: repoRoot,
          env: processEnv,
          stdio: ["ignore", "pipe", "pipe"],
      ...
      try {
        ...
        if (secondProcess?.exitCode === null) {
          await waitFor(
            () =>
              !existsSync(lockfilePath) &&
              readTextFile(fixture.root, "app.txt") === "initial"
          );
        }
    ...
    test("different repos do not replace each other", async () => {
      ...
      try {
        ...
        await waitFor(
          () =>
            readTextFile(firstFixture.root, "app.txt") === "frontyard-second"
        );
        ...
        await waitFor(() => firstProcess.exitCode !== null);
        expect(readTextFile(firstFixture.root, "app.txt")).toBe(
          "frontyard-initial"
        );
        ...
        expect(readTextFile(secondFixture.root, "app.txt")).toBe(
          "chat-initial"
        );
      } finally {
        if (firstProcess.exitCode === null) {
